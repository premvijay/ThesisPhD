...
\begin{enumerate}
    ...
    \item Simulations and analysis methods
    \item Findings
...
% Main Nodes
\node (intro) {Chapter 1: Introduction};
\node (methods) [below=of intro] {Chapter 2 : Methods \\ Modern realistic simulations\\ of relaxation response};
\node (results1) [right=of methods, xshift=-0.5cm, yshift=2cm] {Chapter 3: Diverse and \\universal aspects of relaxation \\across a variety of haloes};
\node (results2) [right=of methods, xshift=-0.5cm, yshift=0.1cm] {Chapter 4: Role of \\Astrophysical Modeling in \\ simulated response};
\node (results3) [right=of methods, xshift=-0.5cm, yshift=-1.7cm] {Chapter 5 : Timescales \\and Relaxation dynamics};
\node (tractable) [below=of methods, xshift=0cm, yshift=0.2cm] { In more tractable \\halo-galaxy systems };
\node (selfsim) [below=of results3, yshift=1cm] {Chapter 6: Self-Similar Model};
\node (conclusion) [below=of tractable, yshift=1.2cm] {Chapter 7: Conclusions };
% Connections
\draw [->] (intro) -- (methods);
...
% \path (results1) -- (selfsim);
% \path (results2) -- (selfsim);
% \path (results3) -- (selfsim);
% \path (selfsim) -- (conclusion);
\end{tikzpicture}
\end{frame}
...
% \begin{frame}
% \begin{tikzpicture}
% \node (top) [rectangle, draw] {Chapters};
% \node (sub) [rectangle, draw, right of=top, xshift=4cm] {Introduction};
% \node (met) [rectangle, draw, xshift=2cm, yshift=-2cm] {Simulations and Methods};
% \node (selfsim) [rectangle, draw, yshift=-4cm] {Controlled numerical experiment};
% \draw [->] (top) -- (sub);
% \draw [->] (top) -- (met);
% \end{tikzpicture}
% \end{frame}
...
\begin{frame}{Diversity in relaxation relation across halo masses}
        \begin{columns}
            ...
            \begin{column}{0.59\linewidth}
                \only<1>{\centering {\LARGE IllustrisTNG} \\ \includegraphics[width=\linewidth]{plots/fit_view_M_T.pdf}}
                \pause
                \only<2>{\centering {\LARGE EAGLE} \\ \includegraphics[width=\linewidth]{plots/fit_view_M_E.pdf}}
            \end{column}
        \end{columns}
\end{frame}
...
\begin{frame}{Applications to }
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item Using the three parameter model, we can predict the relaxed dark matter profile given the baryonic mass profiles.
                \item These predictions are an order of magnitude better than existing models.
            \end{itemize}
        \end{column}
        \begin{column}{0.48\linewidth}
...
\begin{frame}
    ...
    \begin{column}{0.5\linewidth}
    \begin{itemize}
        \item Relaxation offset parameter $q_0$ as a function of the baryonic feedback parameters in CAMELS-TNG. Left: $z=1$, Right: $z=0$.
        \item  Using the suite of CAMELS simulations with variation in feedback strengths, we found that the offset parameter $q_x$, which is inversely related to $q_0$, indeed strongly depends on the overall feedback flux from the galaxies. However, the parameters controlling the feedback burstiness and the wind speed have negligible impact on the relaxation offset as shown below.
    \end{itemize}
    \end{column}
    \begin{column}{0.48\linewidth}
        \includegraphics[width=0.99\linewidth]{plots/CAMELS_I_q0_sn18.pdf}
        \includegraphics[width=0.99\linewidth]{plots/CAMELS_I_q0_sn33.pdf}
    \end{column}
    \end{columns}
\end{frame}
