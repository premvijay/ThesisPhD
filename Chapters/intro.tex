...
\quotehere{
    Nature may be just a random collection of information becoming more random as we go forward in time following simple fundamental rules. The real beauty is with some rarely occurring but self-sustaining patterns that tends to accelerate the overall randomisation; producing every interesting thing at different scales such as galaxies, stars and the biological life on earth. However, randomness will eventually kill every such pattern at some point in time, but the same randomness causes some of these patterns to mutate into different patterns that continues to survive selected by the nature. Among these patterns, the intelligent ones such as humans are one step further by creating correlations with the nature through a process called learning. More specifically science is an organisation of creating progressively more correlations with the nature. Cosmology involves creating such correlations with patterns at the largest known scales of the Universe. A cosmological simulation is an evolution of these correlated patterns at the human scales that leads to the creation of more human scale patterns with ever increasingly stronger correlations with the largest scales of the Universe.
}
